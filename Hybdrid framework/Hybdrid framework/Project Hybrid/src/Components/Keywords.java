...
import static Components.DriverScript.APP_LOGS;
import static Components.DriverScript.CONFIG;
import static Components.DriverScript.OR;
import static Components.DriverScript.currentTestSuiteXLS;
...
public class Keywords {
	...
	//Click the menu of Ametek
	public String clickMenu(String object, String data) {
		APP_LOGS.debug("Clicking a menu item ");
		try {
			// System.out.println("in clicklink");
	...
	// To click a button or element when clickButton is not working, it works
	// like pressing enter on a link
	...
	// To select randomly data from a list as well as data, but prefer using
	// slectListNew keyword
	...
	// To select an element from a list when List contains integer values,please
	// suffix "k" after each value in sheet
	...
	// To select a radio button with pattern of integer value, Do the same as in
	// above keyword
	...
	public String verifyAllListElements(String object, String data) {
		...
		try {
			...
			// define all expected value in OR separated by "," each and extract
			// the expected values from OR. properties
			String temp = data;
	 ...
	 * APP_LOGS.debug("Verifying All checkbox selected"); String checked=null;
	 * String temp[]=object.split(Constants.DATA_SPLIT);
	 * 
	 * for(int i=2;i<=Integer.parseInt(data);) {
	 * checked=driver.findElement(By.xpath(temp[0]+i+temp[1])).getAttribute(
	 * "checked"); if(checked!=null) return Constants.KEYWORD_PASS; else return
	 * Constants.KEYWORD_FAIL +i+ " - Not selected";
	 * 
	 * 
	 * }
	 * 
	...
	public String clickImage(String object, String data) {
		...
		try {
			// String
			// src=driver.findElement(By.xpath(object)).getAttribute("src");
	...
	/*
	 * public String synchronize(String object,String data){ APP_LOGS.debug(
	 * "Waiting for page to load"); ((JavascriptExecutor) driver).executeScript(
	 * "function pageloadingtime()"+ "{"+ "return 'Page has completely loaded'"+
	...
	// method to generate unique number and appending generated number with some
	// static value to generate unique value every time
	...
	public String synchronize(String object, String data) {
		APP_LOGS.debug("Waiting for the page to load");
	...
	// Validate all the items in the xpath
	public String validateAllMenuItems(String object, String data) {
		APP_LOGS.debug("Validating all the menu items");
		try {
			// WebElement webelement = driver.findElement(By.xpath(object));
			List<WebElement> menu_items = driver.findElements(By.xpath(object));
			List<String> actualList = new ArrayList<String>();
			for (WebElement webElement2 : menu_items) {
				actualList.add(webElement2.getText().trim().toLowerCase());
			}
			List<String> expectedList = currentTestSuiteXLS.getDataFromColumn(currentTestSuiteXLS, "Validate", data);
			if (actualList.size() != expectedList.size())
				return Constants.KEYWORD_FAIL + "- size of lists do not match";
			for (int i = 0; i < expectedList.size(); i++) {
				if (!actualList.get(i).equals(expectedList.get(i))) {
					return Constants.KEYWORD_FAIL + "- Element not found - " + actualList.get(i);
				}
			}
		} catch (Exception e) {
			return Constants.KEYWORD_FAIL + " - Could not select from list. " + e.getMessage();
	...
	// Verify if an image is present
	public String verifyImagePresence(String object, String data) {
		APP_LOGS.debug("Verifying the image" + object);
		WebElement webelement = driver.findElement(By.xpath(object));
		...
		if (imagePresent)
			...
			return Constants.KEYWORD_FAIL + " - No image found. ";
	}

	// Verify images inside a Web Element
	public String verifyAllImagePresence(String object, String data) {
		APP_LOGS.debug("Verifying prescence of all the images");
		try {
			...
			return Constants.KEYWORD_FAIL + " - No image found. " + e.getMessage();
		}
	}

	// Verify the link in the attribute href
	public String verifyHref(String object, String data) {
		APP_LOGS.debug("Verifying the attribute href in the anchor tag");
		try {
			WebElement webElement = driver.findElement(By.xpath(object));
			String hrefLink = webElement.getAttribute("href");
			if (hrefLink.equals(data)) {
				return Constants.KEYWORD_PASS;
			} else
				return Constants.KEYWORD_FAIL + " - Links are not same";
		} catch (Exception e) {
			return Constants.KEYWORD_FAIL + " - Exception occured" + e.getMessage();
		}
	}
	
	// Verify the status code of the href link
	public String verifyUrlStatus(String object, String data) {
		APP_LOGS.debug("Verifying the URL status of the href link");
		Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("172.22.218.218", 8085));
		URL urlLink = null;
		try {
			urlLink = new URL(driver.findElement(By.xpath(object)).getAttribute("href"));
		} catch (MalformedURLException e) {
			return Constants.KEYWORD_FAIL + " - Exception occured" + e.getMessage();
		}
		HttpURLConnection uc = null;
		try {
			uc = (HttpURLConnection)urlLink.openConnection(proxy);
		} catch (IOException e) {
			return Constants.KEYWORD_FAIL + " - Exception occured" + e.getMessage();
		}
		try {
			...
			if(uc.getResponseCode()!=200){
				return Constants.KEYWORD_FAIL + " The url is not reachable";
			}
			else{
				return Constants.KEYWORD_PASS;
			}
		} catch (Exception e) {
			APP_LOGS.debug("Connection failed when using url.connect()");
			return Constants.KEYWORD_FAIL + " - Exception occured" + e.getMessage();
	...
	public String getDocumentsAndVerifyStatus(String object,String data){
		APP_LOGS.debug("Getting all the links of the documents in the tab and then verifying the status of the links");
		...
		for (WebElement webElement2 : webElements) {
			listOfHrefs.add(webElement2.getAttribute("href"));
		}
		for (String string : listOfHrefs) {
			APP_LOGS.debug("I am verifying href "+string+"");
			Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("172.22.218.218", 8085));
			URL urlLink = null;
				urlLink = new URL(string);
			HttpURLConnection uc = null;
			
				uc = (HttpURLConnection)urlLink.openConnection(proxy);
				uc.connect();
				if(uc.getResponseCode()!=200){
					APP_LOGS.debug(Constants.KEYWORD_FAIL + " The url "+string+ "is not reachable");
					notAvailable = true;
					break;
				}
				else{
	...
	// Verify the status code of the href link
	public String verifyUrlStatusHome(String object, String data) {
		APP_LOGS.debug("Verifying the URL status of the href link");
		//Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("172.22.218.218", 8085));
		URL urlLink = null;
		try {
		...
		HttpURLConnection uc = null;
		try {
			uc = (HttpURLConnection)urlLink.openConnection();
		} catch (IOException e) {
			return Constants.KEYWORD_FAIL + " - Exception occured" + e.getMessage();
		}
		try {
	...
	@SuppressWarnings("null")
	public String getDocumentsAndVerifyStatusHome(String object,String data){
		APP_LOGS.debug("Getting all the links of the documents in the tab and then verifying the status of the links");
		Boolean notAvailable = false;
		try{
		WebElement webElement = driver.findElement(By.xpath(object));
